...
export default connect(({ mtjzxxq, loading }) => ({ ...mtjzxxq }))(
  class Mtcb extends Component {
    ...
    /** 所有的查询方法 */
		QueyrShip(e) {
		  this.props.dispatch({
			// type: 'mtjzxxq/QueryShip', payload: e ? { Todate: e, tabName: 'SCCT_DATA', type: this.state.TERMINALCODE } : { tabName: 'SCCT_DATA', type: this.state.TERMINALCODE }
          ...
          const list = e[e.length - 1];
				...
				this.props.tabs[this.state.index]['data'][0]['val'] = Number(list.list[0][0]['attributes']['GATEIN'])
				this.props.tabs[this.state.index]['data'][1]['val'] = Number(list.list[0][0]['attributes']['GATEOUT'])
				this.props.tabs[this.state.index]['data'][2]['val'] = Number(list.list[0][0]['attributes']['LOADING'])
				this.props.tabs[this.state.index]['data'][3]['val'] = Number(list.list[0][0]['attributes']['DISCHARGE'])
				this.setState({ json: list.list, shipjson: this.props.tabs[this.state.index]['data'] });
				if (list.lastToday['features'].length > 0) {
				  let jzsl = Number(list.lastToweek['features'][0]['attributes']['GATEIN']);
				  let czsl = Number(list.lastToweek['features'][0]['attributes']['GATEOUT']);
				  let zcsl = Number(list.lastToweek['features'][0]['attributes']['LOADING']);
				  let xcsl = Number(list.lastToweek['features'][0]['attributes']['DISCHARGE']);
